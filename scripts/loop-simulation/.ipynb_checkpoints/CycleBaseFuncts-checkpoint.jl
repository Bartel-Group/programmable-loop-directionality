...
--> Returns: k (concatonated array with the forward and reverse rate constants), K (array of Eq. constants)
    ...
    Directionality: calculates the J descriptor for each series reaction in the loop. Averages the 3 descriptors to predict the direction of the loop.
    --> Returns: J_avg (the average of the 3 directionality descriptors)
    !! Not used in the loop simulation !!
